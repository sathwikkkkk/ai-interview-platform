...
app.delete('/api/user/:userId', async (req, res) => {
  try {
    ...
    res.json({ message: 'Account and interview history deleted successfully' });
  } catch (error) {
    res.status(500).json({ message: 'Server error', error: error.message });
  }
});
...
io.on('connection', (socket) => {
  ...
  socket.on('join-interview', async (data) => {
    ...
    if (interview.chatTranscript.length === 0) {
...
try {
  ...
  console.warn('Could not set up static serving for frontend:', err.message);
}
